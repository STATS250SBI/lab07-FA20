...
# Get the number in the 2nd row and 1st column of the table (treatment + buy)
# and subtract the number in the first row and first column (control + buy).
...
pHatDiffs <- replicate(2, {
  ...
  shuffle_table[2, 1] - shuffle_table[1, 1] # p-hat_treatment minus p-hat_control
})
```
...
If there was no treatment effect, then we'd only observe a difference (treatment - control) of -20% or less about 0.7% of the time. This is so rare, that it provides strong evidence that there is a treatment effect.
...
> **60** Using the figure below, estimate the p-value and determine the conclusion of the hypothesis test.
